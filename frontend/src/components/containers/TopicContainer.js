...
export default function TopicScreen({ route, navigation }) {
  ...
  ////////////////// TEST RESET STATE //////////////////
  const [testReset, setTestReset] = useState(false); //  test
  //////////////////////////////////////////////////////
  ...
  const loadQuestions = async () => {
    ...
    try {
      ...
      setQuestions(allQuestions); // fallback
    }
  };

  const handleWrongAnswer = async () => {
    try {
      const res = await axios.post(`${BASE_URL}/energy/reduce`, {
  ...
  const handleAnswer = async (isCorrect, message) => {
    ...
    if (isCorrect) {
      ...
      // Save as correct if not in test mode
      if (!testReset) {
  ...
  const resetTopicForTest = async () => {
    ...
    setTestReset(true); // temporal test
    await loadQuestions();
  };
  /////////////////////////////////////////////////////////
