...
export default function TemporaryMainContainer() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://192.168.1.72:3000/api/childProgress/dashboard/${userId}`)
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://192.168.1.72:3000/api/childProgress/modules?userId=${userId}`)
      .then((response) => setModules(response.data))
      .catch((error) => console.error('Error fetching modules:', error));
  }, [userId]);

  const handleTopicPress = (topic) => {
    navigation.navigate('TopicScreen', { topic });
  };
